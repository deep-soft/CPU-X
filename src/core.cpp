...
static int call_dmidecode(Data &data)
{
	...
	for(auto& cpu_type : data.cpu.cpu_types)
		cpu_type.processor.package.value = msg.cpu_package;
	if(data.cpu.clocks.bus_freq == 0.0)
		data.cpu.clocks.set_bus_freq(double(msg.bus_freq));

	/* Tab Motherboard */
	data.motherboard.board.manufacturer.value = msg.mb_manufacturer;
	data.motherboard.board.model.value        = msg.mb_model;
	data.motherboard.board.revision.value     = msg.mb_revision;
	data.motherboard.bios.brand.value         = msg.bios_brand;
	data.motherboard.bios.version.value       = msg.bios_version;
	data.motherboard.bios.date.value          = msg.bios_date;
	data.motherboard.bios.romsize.value       = string_set_unit(msg.bios_romsize);

	/* Tab RAM */
	for(uint8_t i = 0; i < msg.dimm_count; i++)
	{
		data.memory.grow_dimms_vector();
		data.memory.dimms[i].reference.value = string_set_unit(msg.memory[i]);
	}
...
static int call_bandwidth(Data &data)
{
	...
	else
	{
		...
		if(pthread_mutex_trylock(&bwd.mutex) != EBUSY)
		{
			...
			MSG_DEBUG("call_bandwidth: created new thread");
			bwt.detach();
		}
		else
			MSG_DEBUG("call_bandwidth: a previous thread is still running");
	}
...
static bool gpu_is_on(std::string device_path)
{
	bool ret = true;
#ifdef __linux__
...
#else /* __linux__ */
	size_t len               = MAXSTR;
	char sysctl_name[MAXSTR] = "";
	char driver_name[MAXSTR] = "";

	snprintf(sysctl_name, len, "%s.name", device_path);
	if(sysctlbyname(sysctl_name, driver_name, &len, NULL, 0) != 0)
	{
		MSG_ERRNO("set_gpu_kernel_driver(%s): sysctlbyname: sysctl_name=%s", card.device_path.c_str(), sysctl_name);
		return 1;
	}
...
static int set_gpu_user_mode_driver(Data::Graphics::Card &card)
{
	using GpuDrv = Data::Graphics::Card::GpuDrv;
	int err = 0;
...
static int set_gpu_vulkan_version(Data::Graphics::Card &card, struct pci_dev *dev)
{
#if HAS_Vulkan
...
static int set_gpu_compute_unit(Data::Graphics::Card &card, struct pci_dev *dev)
{
	int ret_cl = 0;
...
static int gpu_monitoring(Data &data)
{
	using GpuDrv = Data::Graphics::Card::GpuDrv;
	struct Item
	{
	...
	return (failed_count == data.graphics.cards.size());
#else /* __linux__ */
	UNUSED(data);
	return 0;
#endif /* __linux__ */
...
static int motherboardtab_fallback(Data &data)
{
	int err = 0;
...
# endif /* HAS_LIBCPUID */
	...
	snprintf(name, MAXSTR, "dev.cpu.%i.temperature", data.current_core_id);
	if(!(err = sysctlbyname(name, &temperature, &len, NULL, 0)))
		data.cpu.processor.temperature.value = string_format("%.1f°C", double(temperature - 2731) / 10.0); // decikelvins
#endif /* __linux__ */
...
static int cputab_volt_fallback(Data &data)
{
	int err = 0;
	auto& cpu_type = data.cpu.get_selected_cpu_type();
	const uint16_t current_core_id = data.cpu.get_selected_core_id();
...
static int cputab_freq_fallback(Data &data)
{
	int err = 0;
	const uint16_t current_core_id = data.cpu.get_selected_core_id();
...
int fill_labels(Data &data)
{
	...
	for(TabNumer tab_number = TAB_CPU; tab_number < TAB_ABOUT; tab_number = TabNumer(tab_number + 1))
...
/* Refresh some labels */
int do_refresh(Data &data, TabNumer tab_number)
{
	int err = 0;
