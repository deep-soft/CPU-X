...
static void main_win(WINDOW *win, Data &data)
{
	...
	{
		if(static_cast<TabNumer>(i) == Options::get_selected_page())
		{
			if(Options::get_color())
...
static void ntab_memory(WINDOW *win, Data &data)
{
	...
	/* Banks frame */
	for(const auto& dimm : data.memory.dimms)
	{
		draw_frame(win, start, SizeInfo::start, end, SizeInfo::width - 1, dimm);
		line = start + 1;
		mvwprintw2c(win, line++, 2, "%13s", "%s", dimm.reference);
		start = end + 1;
		end  += frame_height;
	}
}
	...
	while(ch != 'q')
	{
		...
		switch(ch)
		{
			...
			case KEY_UP:
				...
				{
					Options::set_selected_gpu(Options::get_selected_gpu() + 1, data.graphics.cards.size() - 1);
					draw_window(win, data);
				}
				else if((Options::get_selected_page() == TAB_BENCH) && data.bench.parameters.set_duration(data.bench.parameters.duration_i + 1))
					draw_window(win, data);
